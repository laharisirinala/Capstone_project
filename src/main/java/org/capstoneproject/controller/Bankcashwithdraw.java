...
@Controller
public class Bankcashwithdraw {
	@Autowired
	private BankUserDao bankuserDao;

	@RequestMapping(value = "/checkwithdrawal/bankdraw", method = RequestMethod.POST)
	public ModelAndView cashwithdraw(@RequestParam("aid") String aid,@RequestParam("pass") String pass,@RequestParam("amount") int amount, ModelAndView mv) {
		int bal=0;
		...
		int counter = bankuserDao.balwithdraw(aid,pass,y);
		...
		if (counter> 0) {
			mv.addObject("msg", "Amount withdrawal successfully");
			...
			mv.addObject("msg", "Error in Amount Withdrawal");
			mv.addObject("bal",amount);
			mv.setViewName("display");
		}}
		else
		{
			mv.addObject("msg", "Insufficient amount ");
			mv.addObject("bal",amount);
			mv.setViewName("display");
		}
